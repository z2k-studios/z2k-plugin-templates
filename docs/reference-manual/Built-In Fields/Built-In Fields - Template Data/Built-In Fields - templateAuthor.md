...
Like [[Built-In Fields - templateName]], the `{{templateAuthor}}` built-in field will be filled in with the author name of the template used to create a file. 

Note: A template uses the [[YAML Configuration Fields|YAML entry]] `z2k_template_author` to specify its current version.
...
## Example
![[Built-In Fields - templateName#Example]]

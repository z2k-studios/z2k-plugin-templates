...
> - The code comment at line 366 of `z2k-template-engine/src/main.ts` says "fieldInfos and blocks are not supported in reduced-set templates." Clarify whether "blocks" here means block templates (partials) or also Handlebars block helpers (`#if`, `#each`). Code analysis suggests block helpers DO work since they're native Handlebars, but this should be verified with tests.
> - ==Needs testing==: Verify that `{{#if}}` and `{{#each}}` actually work inside fieldInfo parameters (e.g., `prompt="{{#if projectName}}Task for {{projectName}}{{else}}Task name{{/if}}"`). The Block Helpers page currently says they don't work in these contexts.

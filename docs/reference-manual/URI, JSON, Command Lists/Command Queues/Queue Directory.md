...
---
# Queue Directory
The Queue Directory is the folder where the Command Queue watches for new command files. Any `.json` or `.jsonl` file placed here will be picked up and processed by the plugin on the next scan cycle.
...
## Default Location
The default queue directory is:
```
.obsidian/plugins/z2k-plugin-templates/command-queue/
```

This path is vault-relative – it lives inside the plugin's own configuration folder. The plugin creates the directory automatically if it doesn't exist when the queue processor first runs.
...
## Alternative Locations
You can change the queue directory to any location via [[Queue Settings]]. Just be aware that the folder must be in a location accessible to the application and have read/write access. For sandboxed devices like mobile phones, this may limit your options.

### Can I Use a Folder in My Vault?
Yes – and this is often the most convenient option. A vault-relative path like `_queue` or `System/command-queue` keeps everything in one place and makes the queue directory easy to find.
...
Yes – you can point the queue directory to a folder managed by iCloud, Dropbox, OneDrive, or similar services. This enables workflows where one device writes commands and another processes them.
...
## Directory Structure
The queue directory contains command files at the top level and uses two subfolders to organize completed and failed commands:
...
## Placing Files in the Queue
To queue a command, write a `.json` or `.jsonl` file into the queue directory. How you create that file depends on your workflow:
- **Shell script** – use `echo` or `cat` to write JSON into the directory
...
- **Manual** – create the file in any text editor and save it to the directory
...
> - The `updateQueueDirPath()` function has a TODO comment for moving existing files when the directory changes. This is currently unimplemented.
> - The path resolution logic (`resolveQueueFilePath()`) checks for `/` or drive letter prefixes to distinguish absolute from vault-relative paths. There is no validation that the path actually exists – the queue processor silently skips if the directory is missing.
> - Confirm whether the plugin creates the top-level queue directory on first run, or only when the first command is archived. The `done/` and `failed/` subfolders are created on demand in `getTimestampedPath()`, but the root directory creation behavior should be verified.

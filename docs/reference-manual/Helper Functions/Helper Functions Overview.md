...
In Z2K Templates will also using the naming conventions to help distinguish between helper functions and fields. Helpers use a dash (`-`) to separate words in the name of the helper (e.g. `formatString`) where fields simply collapse the spaces (e.g. `yearMonth`). See [[Naming Fields]] for more details. 

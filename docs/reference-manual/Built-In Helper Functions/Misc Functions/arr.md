...
### Creating an Options List for fieldInfo
The most common use of `arr` is to create options for `singleSelect` or `multiSelect` fields:

```handlebars
{{fieldInfo priority type="singleSelect" options=(arr "Low" "Medium" "High" "Critical")}}
```
...
```handlebars
{{fieldInfo tag1 prompt="First tag"}}
{{fieldInfo tag2 prompt="Second tag"}}
{{fieldInfo tag3 prompt="Third tag"}}

tags: {{arr tag1 tag2 tag3}}
```

### Using with formatStringBulletize

```handlebars
{{formatStringBulletize (arr "First item" "Second item" "Third item")}}
```
...
- [[formatStringBulletize]] for formatting arrays as bulleted lists
- [[fieldInfo (Field Functions)]] for using arrays with field options

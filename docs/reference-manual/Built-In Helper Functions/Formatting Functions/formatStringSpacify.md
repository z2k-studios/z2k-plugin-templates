...
sidebar_label: "{{formatStringSpacify}}"
---
# formatStringSpacify Helper
...
where:
- `formatStringSpacify` is the predefined name of the helper function for formatting back into an expression with spaces
...
## Examples
- `{{formatStringSpacify "camelCase"}}` → `Camel Case`
- `{{formatStringSpacify "HTTPServer"}}` → `HTTP Server`
- `{{formatStringSpacify "parseXMLDocument"}}` → `Parse XML Document`

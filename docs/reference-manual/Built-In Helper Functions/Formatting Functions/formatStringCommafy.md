...
sidebar_label: "{{formatStringCommafy}}"
---
# formatStringCommafy Helper
The `formatStringCommafy` helper joins an array into a comma-separated string with spaces after each comma.

```handlebars
{{formatStringCommafy fieldName}}
```

where:
- `formatStringCommafy` is the predefined name of the helper function
...
The `formatStringCommafy` helper joins items with a comma **followed by a space**, producing more readable output:

...
When you run the [[Insert block template]] command, the plugin:
- Determines your current note's location (the [[Destination Context]])
- Gathers block templates relevant to that context
- Presents a selection modal for you to choose from

If using the [[Partials|partial syntax]] (`{{> block-name}}`), the block is resolved based on [[Partials#Path Resolution|path resolution rules]].
...
3. **System Blocks** - Values from [[System Block Templates]]
...
This means that if a field already exists in your target document's YAML, that value will be used rather than prompting you again.
...
If using [[Partials]] syntax, prompting will defer until the end when prompting is performaed at the document template level. 
...
The rendered body content is inserted at the specified location:

| Location | Behavior |
| -------- | -------- |
| **Cursor position** | Inserted at the current cursor location (default for interactive use) |
| **Selection replacement** | Replaces the currently selected text |
| `file-top` | Inserted at the beginning of the body (after frontmatter) |
| `file-bottom` | Appended to the end of the file |
| `header-top` | Inserted at the beginning of a specified header section |
| `header-bottom` | Inserted at the end of a specified header section |
| Line number | Inserted at a specific line number |

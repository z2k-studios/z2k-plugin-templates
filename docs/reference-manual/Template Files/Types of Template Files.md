...
---
# Types of Template Files
Z2K Templates supports two distinct kinds of template files: Document Templates and Block Templates, which are different than the Content Files in the vault. This reference page steps through the different types, how they are determined, and how are they treated differently:
...
1. **YAML Explicit Declaration**: Does the file explicitly declare its type with the [[z2k_template_type]] YAML configuration setting? If so, it follows this guidance first. 
...
1. When inserting a block template with an Obsidian Command Prompt (e.g. "[[Insert block template]]" and "[[Insert block template using selected text]]"), it will only display block templates to choose from.

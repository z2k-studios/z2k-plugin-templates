...
The easiest and most standard way to make a markdown file be a template file is to move it into a [[Template Folders|templates folder]].  It's that simple. 
...
When using this method, Templates become *embedded* directly into the same folder - and thus you can remove the proliferation of Templates subfolders. When using this method, the template file is still influenced by the [[Destination Context|destination context]] (i.e. to be useable, the template must be located in the vault's folder tree somewhere within the path up to the [[Templates Root Folder]]).
...
A third method for making a template file be discoverable is to use [[Template File Extensions|File Extensions]] and rename the file to have either a  [[Extension .template|.template]] or [[Extension .block|.block]] file extension. As was the case with the previous method, if you use file extensions, there is no longer a need to store the template into a [[Template Folders|templates folder]].

...
| `templatePath`     | Conditional | Vault-relative path to the template. Either `templatePath` or `templateContents` is required. Used only when the file does not yet exist.                          |
| `templateContents` | Conditional | Inline template text. Either `templatePath` or `templateContents` is required. Used only when the file does not yet exist. Output folder defaults to vault root.   |
...
### Ignored Directives
`destDir`, `fileTitle`, `destHeader`, `location`, `blockPath`, and `blockContents` are ignored. The output path is determined entirely by `existingFilePath`.
